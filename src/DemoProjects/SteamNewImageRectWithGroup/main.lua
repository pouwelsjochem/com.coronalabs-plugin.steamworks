...
-- Create a rotating group to be the parent of the below achievment images.
local group = display.newGroup()
group.x = display.contentCenterX
group.y = display.contentCenterY
local function onTimerElapsed(event)
	group:rotate( 30 / display.fps )
end
timer.performWithDelay( 10, onTimerElapsed, -1 )
...
local function updateAchievementImageFor( achievementName )
	...
	if achievementName == "ACH_TRAVEL_FAR_ACCUM" then
		...
		achievementImage1:translate( -display.contentCenterX, -display.contentCenterY )
	...
	elseif achievementName == "ACH_TRAVEL_FAR_SINGLE" then
		...
		achievementImage2 = steamworks.newImageRect( group, imageInfo.imageHandle, 64, 64 )
		achievementImage2.x = display.contentWidth * 0.75
		achievementImage2.y = display.contentCenterY
		achievementImage2:translate( -display.contentCenterX, -display.contentCenterY )
